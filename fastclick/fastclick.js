;(function () {
  ...
  // 构造函数
    ...
    // 如果是安卓机还需要加上这三个事件
		...
		layer.addEventListener('click', this.onClick, true); // 捕获阶段执行 最早执行这个
    ...
    // 如果已经绑定了onclick事件,将会提前触发,所以fastclick会把这个事件通过addEventListener监听,这样保证click事件
    // 先被fastclick处理
	...
	*/
    ...
    // 不考虑多指操作,如果两个手指都在fastclick元素上，将阻止收缩缩放（问题111）。
